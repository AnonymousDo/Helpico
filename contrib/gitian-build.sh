...
then
	# Make output folder
	mkdir -p "./helpico-binaries/${VERSION}"
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../helpico/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/helpico-*win64-setup.exe ../helpico-binaries/${VERSION}
	    mv build/out/helpico-*win32-setup.exe ../helpico-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../helpico/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/helpico-osx-signed.dmg ../helpico-binaries/${VERSION}/helpico-${VERSION}-osx.dmg
	fi
	popd || exit
