...
*
*   Why is this needed?
*   - In Helpico some blocks are superblocks, which output much higher amounts of coins
...
CAmount GetDevelopmentBudgetPayment(int nHeight, Consensus::Params params) {
  if (nHeight <= 28100) {
      ...
      return nHeight >= 28101 ? 0.45 * GetBlockSubsidy(nHeight, params) : 0;
  }
...
{
    if(fLiteMode) return; // disable all Helpico specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f HELP\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
