...
const char * const BITCOIN_CONF_FILENAME = "helpico.conf";
const char * const BITCOIN_PID_FILENAME = "helpicod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "helpico" is a composite category enabling all Helpico-related debug output
            if(ptrCategory->count(string("helpico"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HelpicoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HelpicoCore
    // Mac: ~/Library/Application Support/HelpicoCore
    // Unix: ~/.helpicocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HelpicoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HelpicoCore";
#else
    // Unix
    return pathRet / ".helpicocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty helpico.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override helpico.conf
