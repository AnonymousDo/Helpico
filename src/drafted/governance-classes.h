 ...
 *
 *   // network
 *   CHelpicoNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.helpico.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CHelpicoNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CHelpicoNetwork(UniValue objIn)
    {
...
// // can be under: HelpicoNetwork
// //   -- signature requirements : Key1(User)
// class CHelpicoNetworkVariable : public CGovernanceObject
// {
// private:
